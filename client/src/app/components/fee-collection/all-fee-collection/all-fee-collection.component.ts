...
@Component({
  selector: 'app-all-fee-collection',
  templateUrl: './all-fee-collection.component.html',
  styleUrls: ['./all-fee-collection.component.scss']
})

  export class AllFeeCollectionComponent implements OnInit{
    ...
    constructor(private router: Router, private ngxService: NgxUiLoaderService,
      private route: ActivatedRoute, private studentService: StudentService, private toastr: ToastrService,
    ...
    setPage(pageInfo: any) {
      ...
      this.subscription = this.feeService.getAll(this.page, this.searchKeyword).subscribe(pagedData => {
      ...
      if (this.selected.length > 0) {
       debugger
        for (let i = 0; i < this.selected.length; i++) {
          const x = this.selected[i];
          if (x.paid_status) {
            $('#confirmation-modal').modal('hide');
            this.toastr.error('Post Transaction not to be delete');
            return;
          }
        }
        ...
        this.feeService.deleteTransaction(body).subscribe(data => {
